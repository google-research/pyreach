...
class TestPyreachArmImpl(unittest.TestCase):
  ...
  def test_execute_actions(self) -> None:
    rdev, dev = self._init_arm("ur5e.urdf", [
        ...
        np.array([
            1.6884295897680983, -1.069162881214778, 1.3461855247223613,
            -1.831992828413365, 4.649924476590007, 0.04101108841917078
        ]),
        np.array([
            1.6689334732548802, -0.9353105405968296, 1.5923844516619943,
            -2.3479616995367163, 4.659719988636331, -0.04306605496813787
        ]),
        np.array([
            1.2961348132487287, -1.3726978968667807, 1.8065409466374114,
            -2.006498799380958, 4.738844597653243, -0.3114337275608076
        ]),
        np.array([
            1.9674813520295258, -1.015049446727209, 1.5155814109025347,
            -2.1164757913300885, 4.653924742768991, 0.3701123824184691
        ]),
        np.array([
            1.1935067871774367, -1.6804996278846147, 2.1172085011547863,
            -2.080637069621664, 4.7020953268829455, -0.4029183934324201
        ]),
    ])
    with test_utils.TestDevice(rdev) as test_device:
      test_device.set_responder(TestArm(""))
      ...
      if state is None:
        return
      status = dev.execute_action("SingulateLeftBin", [
          arm.ActionInput(None, state.pose.position,
                          state.pose.orientation.quaternion)
      ], "test-intent", "test-pick-id", "test-success-type", False)
      self.assertEqual(status.status, "done")
      self.assertEqual(status.error, "")
      test_device.expect_command_data([
          types_gen.CommandData(
                  ...
                  commands=[
                      ...
                      types_gen.ReachScriptCommand(
                          set_output=types_gen.SetOutput(
                              args=[types_gen.CapabilityState(int_value=1)],
                              py_type="vacuum")),
                      types_gen.ReachScriptCommand(
                          set_output=types_gen.SetOutput(
                              args=[types_gen.CapabilityState()],
                              py_type="blowoff")),
                      types_gen.ReachScriptCommand(
                          move_l_path=types_gen.MoveLPathArgs(waypoints=[
                              types_gen.MoveLWaypointArgs(
                                  acceleration=4.0,
                                  rotation=[
                                      1.6884295897680983, -1.069162881214778,
                                      1.3461855247223613, -1.831992828413365,
                                      4.649924476590007, 0.04101108841917078
                                  ],
                                  velocity=1.0)
                          ])),
                      types_gen.ReachScriptCommand(
                          move_l_path=types_gen.MoveLPathArgs(waypoints=[
                              types_gen.MoveLWaypointArgs(
                                  acceleration=4.0,
                                  rotation=[
                                      1.6689334732548802, -0.9353105405968296,
                                      1.5923844516619943, -2.3479616995367163,
                                      4.659719988636331, -0.04306605496813787
                                  ],
                                  velocity=1.0)
                          ])),
                      types_gen.ReachScriptCommand(
                          move_j_path=types_gen.MoveJPathArgs(waypoints=[
                              types_gen.MoveJWaypointArgs(
                                  acceleration=8.0,
                                  rotation=[
                                      1.2961348132487287, -1.3726978968667807,
                                      1.8065409466374114, -2.006498799380958,
                                      4.738844597653243, -0.3114337275608076
                                  ],
                                  velocity=2.0)
                          ])),
                      types_gen.ReachScriptCommand(
                          move_j_path=types_gen.MoveJPathArgs(waypoints=[
                              types_gen.MoveJWaypointArgs(
                                  acceleration=8.0,
                                  rotation=[
                                      1.9674813520295258, -1.015049446727209,
                                      1.5155814109025347, -2.1164757913300885,
                                      4.653924742768991, 0.3701123824184691
                                  ],
                                  velocity=2.0)
                          ])),
                      types_gen.ReachScriptCommand(
                      ...
                      types_gen.ReachScriptCommand(
                          move_j_path=types_gen.MoveJPathArgs(waypoints=[
                              types_gen.MoveJWaypointArgs(
                                  acceleration=8.0,
                                  rotation=[
                                      1.1935067871774367, -1.6804996278846147,
                                      2.1172085011547863, -2.080637069621664,
                                      4.7020953268829455, -0.4029183934324201
                                  ],
                                  velocity=2.0)
                          ])),
                      types_gen.ReachScriptCommand(
  ...
  def test_async_execute_actions(self) -> None:
    rdev, dev = self._init_arm("ur5e.urdf", [
        np.array([
            1.6763853648318705, -0.9809819685779639, 1.5026367936800202,
            -2.0773793705696866, 4.649738410960897, 0.02894469760442231
    ...
    with test_utils.TestDevice(rdev) as test_device:
      ...
      capturer: "thread_util.CallbackCapturer[core.PyReachStatus]"
      capturer = thread_util.CallbackCapturer()
      dev.async_execute_action("SingulateLeftBin", [
          arm.ActionInput(None, state.pose.position,
                          state.pose.orientation.quaternion)
      ], "test-intent", "test-pick-id", "test-success-type", False,
                               callback=capturer.callback,
